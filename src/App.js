...
import audio15 from './Song/Alan Walker - Faded.mp3';
...
const App = () => {
  ...
  const audioSources = useMemo(() => {
    ...
    // Sort the audio sources by artist name in alphabetical order
    return sources.sort((a, b) => a.artist.localeCompare(b.artist));
  }, []);
  ...
  return (
    <Router>
      <div className={`container ${isInputVisible ? 'search-sidebar-open' : ''}`}>
        <NavBar />
        <Routes>
          <Route path="/" element={<Home audioSources={audioSources} isSidebarOpen={isSidebarOpen} />} />
          <Route path="/Contact" element={<Contact />} />
          <Route path="/About" element={<About />} />
          {audioSources.map((source) => (
            <Route 
              key={source.id}
              path={`/${source.artist.replace(/\s+/g, "-")}`} 
              element={<ArtistSongs 
